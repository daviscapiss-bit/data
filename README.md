...
-- AutoFarm para Farming and Friends
-- Funções: Arar, Plantar, Colher, Vender e Comprar sementes

-- Configurações
local seedName = "Corn" -- Nome da semente
local quantidadeComprar = 20 -- Quantas sementes comprar por vez

-- Função para arar o campo
function Arar()
    for _, plot in pairs(game:GetService("Workspace").Plots:GetChildren()) do
        if plot.Owner.Value == game.Players.LocalPlayer then
            for _, tile in pairs(plot.Tiles:GetChildren()) do
                if tile.TileState.Value == "Grass" then
                    game.ReplicatedStorage.Remotes.TillPlot:FireServer(tile)
                    wait(0.1)
                end
            end
        end
...
function Plantar()
    for _, plot in pairs(game:GetService("Workspace").Plots:GetChildren()) do
        if plot.Owner.Value == game.Players.LocalPlayer then
            for _, tile in pairs(plot.Tiles:GetChildren()) do
                if tile.TileState.Value == "Tilled" then
                    game.ReplicatedStorage.Remotes.PlantSeed:FireServer(tile, seedName)
                    wait(0.1)
                end
            end
        end
...
function Colher()
    for _, plot in pairs(game:GetService("Workspace").Plots:GetChildren()) do
        if plot.Owner.Value == game.Players.LocalPlayer then
            for _, tile in pairs(plot.Tiles:GetChildren()) do
                if tile.TileState.Value == "Grown" then
                    game.ReplicatedStorage.Remotes.HarvestPlot:FireServer(tile)
                    wait(0.1)
                end
            end
        end
...
-- Função para vender colheita
function Vender()
    local Market = game:GetService("Workspace").Market
    game.ReplicatedStorage.Remotes.SellAll:InvokeServer(Market)
end

-- Função para comprar sementes
function ComprarSementes()
    local Shop = game:GetService("Workspace").Shop
    game.ReplicatedStorage.Remotes.BuySeeds:InvokeServer(seedName, quantidadeComprar)
end
